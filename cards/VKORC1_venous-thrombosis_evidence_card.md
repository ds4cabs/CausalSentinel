...
**Verdict:** GO — VKORC1 is the validated, clinically approved molecular target of coumarin anticoagulants (such as warfarin) used to treat and prevent venous thrombosis.
...
| Clinical variants | `get_clinvar_variants` | 76 ClinVar records; 3 pathogenic in a sample of 30 |
| Population constraint / LoF tolerance | `get_gnomad_constraint` | pLI=0.00028, LOEUF=1.31 → LoF-tolerant |
| Extra genetic evidence | `get_gwas_catalog` | 103 unique SNPs from 226/226 association rows |
...
- **`get_clinvar_variants`** — Pathogenic count is over the 30 record(s) retrieved, NOT over all 76 ClinVar records for this gene; it is a sample, not a rate.
...
The Open Targets and clinical evidence records confirm that drugs targeting VKORC1, including warfarin sodium, have achieved regulatory approval for venous thrombosis. The target is heavily documented in PharmGKB with multiple clinical annotations connecting genetic variation to drug dosing and toxicity, reflecting its established pharmacological role. Although no pQTL-based Mendelian randomization estimate was available for venous thrombosis, the target's extensive clinical validation and established ChEMBL modulators establish its therapeutic utility.
...
- `get_clinvar_variants`: https://www.ncbi.nlm.nih.gov/clinvar/?term=VKORC1%5Bgene%5D — _ClinVar build Build260818-0035.1_
